...
export default function AddProject() {
  ...
  useEffect(() => {
    ...
    fetch(`${API_BASE}/api/destinations`).then(res => res.json()).then(setDestinations);
    fetch(`${API_BASE}/api/property-types`).then(res => res.json()).then(data => {
      setPropertyTypes(data);
      if (!id && data.length > 0) {
        setFormData(prev => ({ ...prev, type: data[0].name }));
      }
    });
    fetch(`${API_BASE}/api/amenities`).then(res => res.json()).then(setAmenitiesList);
    ...
    if (id) {
      ...
      fetch(`${API_BASE}/api/projects/${id}/amenities`)
        .then(res => res.json())
        .then(data => {
